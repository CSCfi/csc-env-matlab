...
singularity exec --bind=/run/user --bind="$PWD" "$MATLAB_CONTAINER" bash -c "
cp -f /opt/matlab/bin/mw_mpiexec $PWD/bin/mw_mpiexec
cp -f /opt/matlab/bin/mw_smpd $PWD/bin/mw_smpd
...
cp -f /opt/matlab/bin/glnxa64/mpiexec2 $PWD/bin/glnxa64/mpiexec2
cp -f /opt/matlab/bin/glnxa64/smpd $PWD/bin/glnxa64/smpd
